...
% TP: TODO: rozwincie tę myślę; zazwyczaj agentem jest program/algorytm, a tutaj chyba człowiek - bo to przecież Wy modyfikowaliście prompt na podstawie uzyskanych wyników; To jest nietypowe, kiedy mówimy o uczeniu ze wzmocnieniem w kontekście uczenia maszynowego. Ja bym to nawet bardziej traktował jak inżynierię podpodpowiedzi niż uczenie ze wzmocnieniem.
...
\textit{OptiMUS} \cite{ahmaditeshnizi2023optimus} jest systemem opartym o \akronim{DMJ}, który generuje modele PL w języku TODO na podstawie opisu słownego zagadnienia w języku angielskim. % TP: TODO <- uzupełnijcie język/format, w którym działa OptiMUS
Głównymi różnicami między narzędziami jest obsługa modeli \english{mixed-integer linear programming, MILP} oraz system automatycznej iteracji w~celu naprawy błędów (\textit{OptiMUS}). % TP: TODO <- nie jest jasne, który system obsługuje MILP, a który nie; wydawało mi się, że Wasz obsługuje MILP; z punktu widzenia generowania kodu obsługa MILP nie jest szczególnie trudna.
...
Program generuje wtedy poprawiony kod --- operacja jest wykonywana do momentu uzyskania poprawnej odpowiedzi, bądź do momentu osiągnięcia maksymalnej liczby iteracji. Dodatkowo \textit{OptiMUS} zapisuje dane z tekstu do pliku w określonym formacie, co ułatwia przetwarzanie. % TP: TODO <- jakie dane? Czy to jest istotna informacja?
